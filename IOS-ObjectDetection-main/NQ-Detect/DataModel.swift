...
final class DataModel: NSObject,ObservableObject {
    let camera = Camera() //สร้างตัวแปรกล้องด้วย camera
    var detector = ObjectDetection() //สร้างตัวแปรตรวจจับวัตถุด้วย detector
    var labeler = Labeling() //สร้างตัวแปรวาดบล็อกด้วย labeler
    
    @Published var viewfinderImage: Image? // รูปภาพที่แสดงใน Viewfinder
    @Published var thumbnailImage: Image? // รูปภาพขนาดย่อสำหรับแสดงใน Sheet
    
    var isPhotosLoaded = false //สถานะการโหลดรูปภาพ
    
    override init() { // เริ่มต้นทำงานเมื่อสร้างวัตถุ
        super.init()
        Task {
